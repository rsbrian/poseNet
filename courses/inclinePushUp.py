...
class Action(object):
    ...
    def __call__(self):
        print("Action")
        if self.brain.is_pose("prepare_action_huge"):
            print("請回到預備動作重新開始")
        ...
        elif self.brain.is_pose("hands_up_shoulder"):
            # print("Bar1 Open")
...
class HandsUp(object):
    ...
    def __call__(self):
        ...
        self.counter.start()
        if self.brain.is_pose("ending_shoulder"):
            if self.is_time_small_than(0.8):
                print("你沒有要開始就不要亂動")
            self.course.api.course_action["action"]["alert"] = ["不夠下去不列入次數"]
            self.course.set_time("alertLastTime")
            self.course.set_time("startPointLastTime")
            self.course.change(Action(self.course, self.brain))

        elif self.brain.is_pose("prepare_action_huge"):
            print("請回到預備動作重新開始")
            self.course.api.course_action["action"]["alert"] = [
                "請回到預備動作重新開始"]
            self.course.set_time("alertLastTime")
        ...
        elif self.brain.is_pose("hands_down_incline"):
