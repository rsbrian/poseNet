...
class OneArmDubbleRowLeft(Home):
...
class Prepare(PrepareTemp):
    prepare_notes = {
        ...
        #"請將右手至於大腿上並將身體向前傾": "right_hand_feet",
        "將左手自然垂放": "drop_hand_natrually_left"
    }
...
class Action(object):
    ...
    def __call__(self):
        ...
        if self.brain.is_pose("front_right_feet"):
            # print("請將右腳往前踏左腳向後伸​")
            self.course.api.course_action["action"]["alert"] = ["請將右腳往前踏左腳向後伸​"]
            self.course.set_time("alertLastTime")
            self.course.set_time("startPointLastTime")
            self.course.change(
                ErrorHandleing(self.course, self.brain))
        elif self.brain.is_pose("left_elbow_expansion"):
            print("左手肘外擴了，請回到預備動作重新開始")
            ...
            self.course.change(
                ErrorHandleing(self.course, self.brain))
        elif self.brain.is_pose("hands_up_left"):
            # print("Bar1 Open")
...
class HandsUp(object):
    ...
    def __call__(self):
        ...
        self.counter.start()
        if self.brain.is_pose("ending_left"):
        ...
        elif self.brain.is_pose("front_right_feet"):
            # print("請將右腳往前踏左腳向後伸")
            self.course.api.course_action["action"]["alert"] = ["請將右腳往前踏左腳向後伸"]
            self.course.set_time("alertLastTime")
            self.course.set_time("startPointLastTime")
        ...
        elif self.brain.is_pose("hands_down_boat_left"):
