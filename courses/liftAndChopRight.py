...
class Prepare(PrepareTemp):
    prepare_notes = {
        "持啞鈴於腹前": "hold_dumbbells_on_abdomen"
    }
...
class Action(object):
    ...
    def __call__(self):
        ...
        if self.brain.is_pose("keep_body_stable_right"):
            # print("請保持身體穩定​")
...
class HandsUp(object):
    ...
    def __call__(self):
        ...
        elif self.brain.is_pose("keep_body_stable_right"):
            # print("請保持身體穩定​")
            self.course.api.course_action["action"]["alert"] = ["請保持身體穩定​"]
            self.course.set_time("alertLastTime")
            self.course.set_time("startPointLastTime")
        ...
        elif self.brain.is_pose("hands_down_lift_right"):
