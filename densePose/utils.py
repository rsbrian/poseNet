...
def scale_up(scale, images):
    ...
    for i in range(len(images)):
        ...
        scaled.append(resize(img, output_shape, order=1,
...
def display_images(outputs, inputs=None, gt=None, is_colormap=True, is_rescale=True):
    ...
    for i in range(outputs.shape[0]):
        ...
        result = np.uint8(imgs[1] * 255)
        img_set = np.hstack(imgs)
        all_images.append(img_set)

    all_images = np.stack(all_images)
    return skimage.util.montage(all_images, multichannel=True, fill=(0, 0, 0)), result
...
def evaluate(model, rgb, depth, crop, batch_size=6, verbose=False):
    ...
    for i in range(N//bs):
        ...
        pred_y = scale_up(2, predict(model, x/255, minDepth=10,
                                     maxDepth=1000, batch_size=bs)[:, :, :, 0]) * 10.0

        # Test time augmentation: mirror image estimate
        pred_y_flip = scale_up(2, predict(
            model, x[..., ::-1, :]/255, minDepth=10, maxDepth=1000, batch_size=bs)[:, :, :, 0]) * 10.0
