...
def load_variables(chkpoint, base_dir=BASE_DIR):
    ...
    if not os.path.exists(manifest_path):
        print('Weights for checkpoint %s are not downloaded. Downloading to %s ...' % (
            chkpoint, base_dir))
...
def build_network(image, layers, variables):
    ...
    def _conv_to_output(mobile_net_output, output_layer_name):
        w = tf.nn.conv2d(mobile_net_output, _weights(
            output_layer_name), [1, 1, 1, 1], padding='SAME')
        w = tf.nn.bias_add(w, _biases(output_layer_name),
                           name=output_layer_name)
        return w

    def _conv(inputs, stride, block_id):
        return tf.nn.relu6(
            tf.nn.conv2d(inputs, _weights(
                "Conv2d_" + str(block_id)), stride, padding='SAME')
    ...
    with cg.as_default():
        ...
        with tf.Session() as sess:
            ...
            sess.run(
                ...
                feed_dict={
                    image_ph: [np.ndarray(
                        shape=(height, width, 3), dtype=np.float32)]
                }
            )
            ...
            freeze_graph(
                input_graph=os.path.join(
                    model_dir, "model-%s.pbtxt" % chkpoint),
                input_saver="",
                input_binary=False,
